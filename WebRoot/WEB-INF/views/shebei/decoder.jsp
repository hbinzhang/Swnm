...
<script type="text/javascript">
	$(function(){
		...
		$('#decoderimport').click(function(d){
			var file = $('#decoderfile').val();
			if(!file){
				alert('请选择文件再上传');
				return;
			}
			$('#data_loading').show();
			$('#decoderfileform').ajaxSubmit({
	...
	function dopageup(){
		...
		window.location = '<%=basePath%>videomonitor/loadDecoder.action?pageNo=' + pageNo;
	}
	function dopagedown(){
	...
	function doskip(){
		...
		window.location = '<%=basePath%>videomonitor/loadDecoder.action?pageNo=' + pn;
	}
	
	function toend(){
		var endpage = '${pageCount}';
		window.location = '<%=basePath%>videomonitor/loadDecoder.action?pageNo=' + endpage;
	}
</script>
</head>
<body>
	...
	<div id="main_content_container">
		<div id="nav_place">
			...
			<ul>
				...
				<li><a href="<%=basePath %>videomonitor/loadDecoder.action">解码器设备</a>
				</li>
			</ul>
		</div>
		<div id="content_container">
			...
			<div class="search_container">
				<div class="left" style="margin-top:5px;">
					<form id="dsearchform" action="<%=basePath %>videomonitor/loadDecoder.action" method="post">
				...
				<div class="left" style="margin-top:5px;">
					<input id="dsearch" type="submit" class="btn" value="查询"
						style="margin-left:10px;"> 
						<c:if test="${poteviofn:clContains(sessionScope.session.authorizatedOps,'添加解码器') }">
						<input type="button"
						class="btn" value="添加" id="decoder_addbtn"
						style="margin-left:10px;">
						</c:if>
						<c:if test="${poteviofn:clContains(sessionScope.session.authorizatedOps,'批量导入解码器') }">
						 <input type="button"
						class="btn" value="批量导入" style="margin-left:10px;" id="showimport">
						</c:if>
						<c:if test="${poteviofn:clContains(sessionScope.session.authorizatedOps,'删除解码器') }">
						<input type="button" class="btn" value="删除" style="margin-left:10px;" id="decoderdelete">
						</c:if>
						...
						<div style="position:absolute;border:lightblue solid 1px;background:white;width:280px;height:100px;display:block;display:none;background:white;margin-left:280px;" id="filediv">
							<form id="decoderfileform" action="<%=basePath%>videomonitor/batchImportDecoder.action" enctype="multipart/form-data" method="post">
							<input type="file" name="decoder" id="decoderfile" style="float:left;margin:20px 30px 10px 30px;"/>
							<input id="decoderimport" type="button" value="导入" class="btn" style="float:left;margin:2px 10px 2px 30px;"/>
			...
			<div class="tableui_container">
				<table class="tableui" id="tdecoders">
					<thead>
						<tr>
							<th width="2%">
					    		<input type="checkbox" id="decoder_select_all"/>
					    	</th>
					...
					<tbody>
						<s:iterator var="decoder" value="decoders" status="st">
							<tr>
								<td align="center">
									<input type="checkbox" class="decoderselected"/>
								</td>
								<td id='tddecoderid'><s:property value="decoderid" /></td>
								...
								<td align="center">
								<c:if test="${poteviofn:clContains(sessionScope.session.authorizatedOps,'修改解码器') }">
								<a href="javascript:void(0);" title="编辑"
									class="edit_decoder editlink"> 编辑 </a>
									</c:if>
									 </td>
							</tr>
						</s:iterator>
