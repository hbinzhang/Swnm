...
// On response  optional callback fun is called with optional user data.
	_getXML: function(url, callback) {
		...
		if(url.indexOf("message")!=-1){
			var urlS = url.substring(0,url.indexOf("message")+8);
			var info  = encodeURIComponent(url.substring(url.indexOf("message")+8,url.lastIndexOf("&")));
			var urlT = url.substring(url.lastIndexOf("&"));
			url = urlS+info+urlT;
		}
		
		if(url.indexOf("login&name")!=-1){
			var urlS = url.substring(0,url.indexOf("name")+5);
			var info  = encodeURIComponent(url.substring(url.indexOf("name")+5,url.lastIndexOf("&")));
			var urlT = url.substring(url.lastIndexOf("&"));
			url = urlS+info+urlT;
