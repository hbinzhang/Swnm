...
jQuery.TreeTable = function (tree_id, heads, tNodes,widths) {
    if ($("#" + tree_id).length > 0) {
        ...
        for (var i = 0; i < column_count; i++) {
            head_html += "<th nowrap width='"+widths[i]+"%'>" + heads[i] + "</th>";
        }
        head_html += "</tr></thead>";
        ...
        for (var i = 0, l = newNodes.length; i < l; i++) {
            var node = newNodes[i];
            body_html += "<tr data-tt-id='" + node.id + "' " + (node.pId ? "data-tt-parent-id='" + node.pId + "'" : "") + "><td nowrap class = 'td_left'><span class='" + (node.isParent ? "parent" : "children") + "'>" + node.name + "</span></td>";
            for (var j = 0; j < column_count - 1; j++) {
                body_html += "<td nowrap>" + (node.td[j] ? node.td[j] : "") + "</td>";
            }
            body_html += "</tr>";
        }
        body_html += "</tbody>";
