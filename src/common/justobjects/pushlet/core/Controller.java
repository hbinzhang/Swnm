...
package common.justobjects.pushlet.core;

import java.io.IOException;
import java.util.Map;

import common.justobjects.pushlet.servlet.Pushlet;
import common.justobjects.pushlet.util.PushletException;
...
public class Controller implements Protocol, ConfigDefs {
	...
	public static Controller create(Session aSession) throws PushletException {
		...
		try {
			controller = (Controller) Config.getClass(CONTROLLER_CLASS,
					"common.justobjects.pushlet.core.Controller").newInstance();
		} catch (Throwable t) {
	...
	public void doCommand(Command aCommand) {
		try {
			...
			// Handle response back to client  peicaiyun modify 15-04-08
			if (eventType.endsWith(Protocol.E_LISTEN)) {
				...
				String waitCircuits = PULL_REFRESH_WAIT_CIRCUITS;
				//System.out.println("---PULL_REFRESH_WAIT_MIN_MILLIS : E_LISTEN "+Config.getLongProperty(minWaitProperty));
				if(getSubscriber().getEventQueue()!=null){
					int i = 0;
					long loopCounts = Config.getLongProperty(waitCircuits);
					do
					{
						if(getSubscriber().getEventQueue().getCountEvents()==0)
						{
							//System.out.println("---PULL_REFRESH_WAIT_MIN_MILLIS ------0--------i="+ i);
							Thread.sleep(Config.getLongProperty(minWaitProperty));
							//System.out.println("---PULL_REFRESH_WAIT_MIN_MILLIS after:"+(System.currentTimeMillis()));
							i++;
						}
						else
						{
							//System.out.println("---PULL_REFRESH_WAIT_MIN_MILLIS ------1---break-----i="+ i);
							break;
						}
					}while(i < loopCounts);
					getSubscriber().fetchEvents(aCommand);
				}//(getSubscriber().getEventQueue()!=null)
				else
				{
					warn("Exception in doCommand(): " + "Subscriber has no eventQueue");
					
					 throw new Exception("no eventQueue");
				}//(getSubscriber().getEventQueue()!=null)
		...
		try {
			...
			if (subject == null) {
				...
				if (to != null) {
					...
					Map<?, ?> mapInfo = Pushlet.mapInfo;

					String from = aCommand.reqEvent.getField(P_FROM);
					String massage = aCommand.reqEvent.getField(P_MESSAGE);
					massage = massage.replaceAll(to, "\n");
					massage = new String(massage.getBytes("ISO-8859-1"), "utf-8");
			
					System.out.println(mapInfo.get(from) + "==" + massage
							+ "==to==>" + mapInfo.get(to));
	...
	protected void doUnsubscribe(Command aCommand) throws IOException {
		...
		Event responseEvent = null;
		try {
			String subscriptionId = aCommand.reqEvent
					.getField(Protocol.P_SUBSCRIPTION_ID);
			if (subscriptionId == null) {
				...
				if (subscription == null) {
					...
					responseEvent = new Event(E_NACK);
					responseEvent.setField(P_ID, session.getId());
					responseEvent.setField(P_REASON, "no subscription for sid="
							+ subscriptionId);
					...
					if (subscription.getLabel() != null) {
						responseEvent.setField(P_SUBSCRIPTION_LABEL,
...
/*
 * $Log: Controller.java,v $ Revision 1.9 2007/11/23 14:33:07 justb core classes
 * now configurable through factory
 * 
 * Revision 1.8 2005/02/28 15:58:05 justb added SimpleListener example
 * 
 * Revision 1.7 2005/02/28 13:05:59 justb introduced join-listen protocol
 * service
 * 
 ...
 * support
 * 
 * Revision 1.4 2005/02/25 15:13:00 justb session id generation more robust
 * 
 * Revision 1.3 2005/02/21 16:59:06 justb SessionManager and session lease
 * introduced
 * 
