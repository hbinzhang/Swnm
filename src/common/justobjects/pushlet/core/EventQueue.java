...
public class EventQueue {
	...
	public synchronized int getCountEvents() {
		return countEvents;
	}

	public synchronized void setCountEvents(int countEvents) {
	...
	public synchronized boolean enQueue(Event item, long maxWaitTime) throws InterruptedException {
		...
		while (isFull()) {
			log.info("enQueue is full ,item.getSubject() is " +item.getSubject());
			
			if (maxWaitTime > 0) {
				// Wait at most maximum time
				wait(maxWaitTime);
				log.info("enQueue is full ,wait maxWaitTime is " +maxWaitTime);
				
				// Timed out or woken; if still full we
				// had bad luck and return failure.
				if (isFull()) {
					System.out.println("enQueue drop an event ,item.getSubject() is " +item.getSubject());
					log.info("enQueue drop an event ,item.getSubject() is " +item.getSubject());
					return false;
				}
			} else {
				wait();
		...
		//判断event类型,peicaiyun modify,record data event except"/pushlet/ping"
		if((item.getEventType().equals(("data")))&&(!item.getSubject().startsWith("/pushlet/ping")))
		{
			setCountEvents(1);
		}
		...
		//System.out.println("===============enQueue two return "+curtime);
		return true;
	}
	...
	public synchronized Event[] deQueueAll(long maxWaitTime) throws InterruptedException {
		...
		SimpleDateFormat sdFormatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String curtime = sdFormatter.format(nowTime);
		System.out.println("===============deQueueAll one para return" +curtime);*/
		while (isEmpty()) {
			if (maxWaitTime >= 0) {
				wait(maxWaitTime);
