...
public class IPCZoneMapHandler implements ZoneCommonHandler {
	
	private final static Log log = LogFactory.getLog(IPCZoneMapHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			 ...
			 */
			IpcZoneMap ipcZoneMap=null;
			try {
				ipcZoneMap = ZoneConvertUtil.getIpcZoneMap(linked);
			} catch (Exception e) {
				errorRowMeg.add("摄像头映射信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkIpcZoneMap(ipcZoneMap,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.ZONE_IPC_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.ZONE_IPC_DATA_NAME, linkeds);
		}
	}
	...
	private void checkIpcZoneMap(IpcZoneMap ipcZoneMap, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(ipcZoneMap);
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("摄像头映射信息:第"+(rowNum+1)+"行:");
