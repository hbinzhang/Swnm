...
public class IntegrationZoneMapHandler implements ZoneCommonHandler {
	
	private final static Log log = LogFactory.getLog(IntegrationZoneMapHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			 ...
			 */
			IntegrationZoneMapBean integrationZoneMapBean=null;
			try {
				integrationZoneMapBean = ZoneConvertUtil.getIntegrationZoneMap(linked);
			} catch (Exception e) {
				errorRowMeg.add("一体化光纤:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkIntegrationZoneMap(integrationZoneMapBean,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.ZONE_INTE_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.ZONE_INTE_DATA_NAME, linkeds);
		}
	...
	private void checkIntegrationZoneMap(IntegrationZoneMapBean integrationZoneMapBean, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(integrationZoneMapBean);
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("一体化光纤:第"+(rowNum+1)+"行:");
