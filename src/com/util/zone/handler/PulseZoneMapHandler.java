...
public class PulseZoneMapHandler implements ZoneCommonHandler {
	
	private final static Log log = LogFactory.getLog(PulseZoneMapHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			 ...
			 */
			PulseZoneMapBean pulseZoneMapBean=null;
			try {
				pulseZoneMapBean = ZoneConvertUtil.getPulseZoneMap(linked);
			} catch (Exception e) {
				errorRowMeg.add("高压脉冲光纤:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkPulseZoneMap(pulseZoneMapBean,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.ZONE_PULSE_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.ZONE_PULSE_DATA_NAME, linkeds);
		}
	}
	...
	private void checkPulseZoneMap(PulseZoneMapBean pulseZoneMapBean, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(pulseZoneMapBean);
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("高压脉冲光纤:第"+(rowNum+1)+"行:");
