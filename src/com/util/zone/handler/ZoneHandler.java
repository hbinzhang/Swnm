...
public class ZoneHandler implements ZoneCommonHandler {
	
	private final static Log log = LogFactory.getLog(ZoneHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			...
			try {
				zoneBean = ZoneConvertUtil.getZone(linked);
			} catch (Exception e) {
				errorRowMeg.add("防区信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkZoneBean(zoneBean,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.ZONE_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.ZONE_DATA_NAME, linkeds);
		}
	...
	private void checkZoneBean(ZoneBean zoneBean, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(zoneBean);
