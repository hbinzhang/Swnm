...
public class PositionZoneMapHandler implements ZoneCommonHandler {
 
	private final static Log log = LogFactory.getLog(PositionZoneMapHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			 ...
			 */
			PositionZoneMapBean positionZoneMapBean=null;
			try {
				positionZoneMapBean = ZoneConvertUtil.getPositionZoneMap(linked);
			} catch (Exception e) {
				errorRowMeg.add("定位型光纤:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkPositionZoneMap(positionZoneMapBean,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.ZONE_POSITION_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.ZONE_POSITION_DATA_NAME, linkeds);
		}
	...
	private void checkPositionZoneMap(PositionZoneMapBean positionZoneMapBean, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(positionZoneMapBean);
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("定位型光纤:第"+(rowNum+1)+"行:");
