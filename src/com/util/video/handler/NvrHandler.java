...
public class NvrHandler implements VideoCommonHandler {
	
	private final static Log log = LogFactory.getLog(NvrHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			...
			TVmNvr nvr=null;
			try {
				nvr = NvrConvertUtil.getTVmNvr(linked);
			} catch (Exception e) {
				errorRowMeg.add("NVR信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkNvrBean(nvr,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.NVR_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.NVR_DATA_NAME, linkeds);
		}
	...
	private void checkNvrBean(TVmNvr nvr, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(nvr);
