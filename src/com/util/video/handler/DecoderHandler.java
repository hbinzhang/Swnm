package com.util.video.handler;
...
public class DecoderHandler implements VideoCommonHandler {
	
	private final static Log log = LogFactory.getLog(DecoderHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			 ...
			 */
			TVmDecoder decoder = null;
			try {
				decoder = DecoderConvertUtil.getTVmDecoder(linked);
			} catch (Exception e) {
				errorRowMeg.add("解码器信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkDecoderBean(decoder,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.DECODER_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.DECODER_DATA_NAME, linkeds);
		}
	...
	private void checkDecoderBean(TVmDecoder decoder, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(decoder);
		
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("解码器信息：第"+(rowNum+1)+"行:");
