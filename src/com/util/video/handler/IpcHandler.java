...
public class IpcHandler implements VideoCommonHandler {
	
	private final static Log log = LogFactory.getLog(IpcHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			...
			TVmIpc ipc=null;
			try {
				ipc = IpcConvertUtil.getTVmIpc(linked);
			} catch (Exception e) {
				errorRowMeg.add("IPC信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkIpcBean(ipc,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.IPC_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.IPC_DATA_NAME, linkeds);
		}
	...
	private void checkIpcBean(TVmIpc ipc, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(ipc);
		
		if(l.size()>0){
			//日志打印
			StringBuffer sb = new StringBuffer();
			for(ConstraintViolation constraintViolation:l){
				sb.append("IPC信息:第"+(rowNum+1)+"行:");
