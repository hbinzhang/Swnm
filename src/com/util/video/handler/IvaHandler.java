...
public class IvaHandler implements VideoCommonHandler {
	
	private final static Log log = LogFactory.getLog(IvaHandler.class);
	...
	public void parse2003Excel(HSSFSheet sheet,
		...
		for (int i = sheet.getFirstRowNum(); i < sheet
			...
			TVmIva iva=null;
			try {
				iva = IvaConvertUtil.getTVmIva(linked);
			} catch (Exception e) {
				errorRowMeg.add("IVA信息:第"+(i+1)+"行:数据类型错误\r\n");
				log.error(sheet.getSheetName()+":"+(i+1)+ "行" +"数据转换错误："+e);
				continue;
			}
			//内容校验
			checkIvaBean(iva,errorRowMeg,i);
		...
		if(errorRowMeg.size()>0){
			//内容校验有错
			errorMessage.put(ExcelConfigInfo.IVA_DATA_ERROR, errorRowMeg);
		}else{
			map.put(ExcelConfigInfo.IVA_DATA_NAME, linkeds);
		}
	...
	private void checkIvaBean(TVmIva iva, List<String> errorRowMeg,
			int rowNum) {
		Validator validator = new Validator();
		List<ConstraintViolation> l =validator.validate(iva);
